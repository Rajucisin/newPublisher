...
let QueueService = class QueueService {
    ...
    `;
        this.db.run(blogPostsQuery, (err) => {
            if (err)
                console.error('[QueueService] Failed to create blog_posts table:', err.message);
        });
        this.db.run(queueQuery, (err) => {
            if (err)
                console.error('[QueueService] Failed to create publishing_queue table:', err.message);
        });
    }
    ...
    `;
        this.db.all(query, [nowStr], async (err, rows) => {
            if (err) {
    ...
    async getLatestPendingQueueItem(userId) {
        return new Promise((resolve, reject) => {
            const query = `
    ...
    async getLatestRejectedQueueItem(userId) {
        return new Promise((resolve, reject) => {
            const query = `
        SELECT q.*, p.title, p.linkedin_post_content 
        FROM publishing_queue q 
    ...
    async appendRejectionFeedback(queueId, feedback) {
        return new Promise((resolve, reject) => {
            ...
            this.db.get(selectQuery, [queueId], (err, row) => {
                if (err) {
                    return reject(err);
                }
